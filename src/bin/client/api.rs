...
pub async fn request_image(
    ...
    match middleware::p2p_send_image_request(&socket, sending_socket.clone(), config, client_ip, client_port, &request_message, received_path.clone()).await {
        Ok(_) => {
            // If the request is successful, spawn a task to receive and save the image move evyrthing first
            
            

            let _ = image_com::receive_image(&socket.clone(), &config, sending_socket, received_path).await;

            // Optionally clear file after request
